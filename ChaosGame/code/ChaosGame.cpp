...
int main()
{
	...
	while (vertices.size() < 3)
	{
		...
		while (window.pollEvent(event))
		{
			if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left)
			{
				...
				CircleShape vertexPt(4.0f);
				vertexPt.setFillColor(Color::Blue);
				vertexPt.setPosition(clickPosition.x - 4.0f, clickPosition.y - 4.0f);
				vertices.push_back(vertexPt);
				window.draw(vertexPt);
				window.display();
			}
		}
	}
	...
	while (window.isOpen())
	{
		...
		//Draw triangle vertices/dots when algorithm is running
